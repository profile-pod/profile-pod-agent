...
func (g *GolangInspector) Inspect(p *details.ProcDetails) (profiler.FlameGraphProfiler, bool) {
	file := fmt.Sprintf("/proc/%s/exe", p.ProcessID)
	_, err := os.Stat(file)
	if err != nil {
	...
	return &profiler.BpfProfiler{}, true
}
